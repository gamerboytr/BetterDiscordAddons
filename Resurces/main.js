((_) => {
	...
	window.onload = function () {
		...
		const createAddonList = (type, ending) => {
			let [header, list] = createLoadingList(`https://raw.githubusercontent.com/gamerboytr/gamerboytr.github.io/master/${type}/README.md`, {
				...
				getChildren: (response) =>
					response
						...
						.map((n) => {
							...
							return (
								name &&
								(!createAddonList.search[type] || [name, description].filter((n) => n).some((s) => s.toLowerCase().indexOf(createAddonList.search[type].toLowerCase()) > -1)) &&
								createAddonCard({
									name: name,
									description: description,
									icon: `<svg width="100%" height="100%" viewBox="0 0 24 24"><path fill="currentColor" d="${
										type == "Plugins"
									...
									resourceUrl: `https://github.com/gamerboytr/gamerboytr.github.io/tree/master/${type}/${fileName}/Resurces`,
									websiteUrl: `https://github.com/gamerboytr/gamerboytr.github.io/tree/master/${type}/${fileName}`,
									sourceUrl: `https://github.com/gamerboytr/gamerboytr.github.io/tree/master/${type}/${fileName}/${fileName}${ending}`,
		...
		for (const data of sections) {
			const item = createElement(
				data.section == "Title" ? `<div class="sidebar-title">${data.title}</div>` : data.section == "Separator" ? `<div class="sidebar-separator"></div>` : `<div class="${formatClassName("sidebar-item", selectedSection == data.section && "selected")}">${data.section}</div>`
			);
